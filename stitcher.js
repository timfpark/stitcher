...
var user = new nitrogen.User({ nickname: 'user', email: "timfpark@gmail.com", password: "xxxxxxxx" });
...
service.authenticate(user, function(err, session, user) {
    ...
    nitrogen.Message.find(session, { from: "51c4ba82292b19f00b0013d4", type: 'image' }, {}, function(err, images) {
        ...
        async.eachLimit(images, 50, function(image, callback) {
            ...
            var createVideoCommand = "ffmpeg -f image2 -r 30 -pattern_type glob -i 'images/*.jpg' -c:v libx264 timelapse.mp4";
